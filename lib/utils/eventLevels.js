...
function eventSegments(event, range, accessors) {
  ...
  var flagRight = accessors.start(event) <= last && accessors.end(event) > last;
  var flagLeft = accessors.start(event) < first;

  var span = _dates.default.diff(start, end, 'day');
  var sp = span - 1.25;
  if(flagRight) sp += 0.75;
  if(flagLeft) sp += 0.5; 
  sp = Math.min(sp, slots);
  sp = Math.max(sp, 0.25);
  return { 
    event: event,
    span: sp,
    left: flagLeft ? padding + 1 : padding + 1.5,
    right: Math.max(flagRight ? padding + span : padding + span - 0.75, 0.25)
  };
}
